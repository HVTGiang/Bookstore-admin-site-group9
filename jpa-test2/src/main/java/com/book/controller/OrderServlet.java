...
public class OrderServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        List<OrderStatus> statusList = Order.statusList();
        request.setAttribute("statusList", statusList);

        List<OrderEntity> orderList = OrderDAO.getAll();
        request.setAttribute("orderList", orderList);

        List<OrderEntity> waitingConfirmList = OrderDAO.findByStatus(1);
        request.setAttribute("waitingConfirmList", waitingConfirmList);

        List<OrderEntity> doneList = OrderDAO.findByStatus(4);
...
//        Lấy id của status được truyền xuống nè
        ...
        OrderStatus selectedStatus = new OrderStatus();
        List<OrderStatus> statusList = Order.statusList();
        request.setAttribute("statusList", statusList);
...
//            request.setAttribute("total",totalBook);
            ...
            selectedStatus = Order.findStatusByID(Integer.parseInt(sID));
            List<OrderEntity> orderList = OrderDAO.findByStatus(Integer.parseInt(sID));
            request.setAttribute("orderList", orderList);
            request.setAttribute("status", selectedStatus);
//            int totalBook=Product.totalBook(bookList);
//            request.setAttribute("total",totalBook);
...
//      Lấy product có id tương ứng ra
        OrderEntity selectedOrder = OrderDAO.find(Integer.parseInt(oID));
        request.setAttribute("order", selectedOrder);
        request.setAttribute("action", "edit");

        UserEntity customer = UserDAO.find(selectedOrder.getIdUser());
        request.setAttribute("customer", customer);

        UserEntity employee = UserDAO.find(selectedOrder.getIdSeller());
        ...
        List<OrderitemEntity> orderItemList = OrderItem.finalOrderItemList(OrderItemDAO.orderItemList(Integer.parseInt(oID)));
...
//            Tạo một đối tượng Product để lưu dữ liệu
            ...
            order = OrderDAO.find(Integer.parseInt(oID));
...
//            Nêu dữ liệu chưa hợp lệ
            if (!order.getReceiveDate().after(order.getCreateTime())) {
                ...
                UserEntity customer = UserDAO.find(order.getIdUser());
                request.setAttribute("customer", customer);

                UserEntity employee = UserDAO.find(order.getIdSeller());
                request.setAttribute("employee", employee);
